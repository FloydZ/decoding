...
public:
	...
	constexpr ConfigLeeBrickell(uint32_t n, uint32_t k, uint32_t w, uint32_t p, uint32_t l) :
		n(n), k(k), w(w), l(l), p(p), m4ri_k(matrix_opt_k(k, n)) {}


	// prints the expected number of loops
	void expected(size_t loops) {
		uint64_t expected_loops = bc(n, w)/ (bc(k, w-p) * bc(l, p));
...
int LeeBrickell(mzd_t *e, const mzd_t *const s, const mzd_t *const A) {
	...
	DEBUG_MACRO(uint64_t outer_loops = 0;)
	DEBUG_MACRO(uint64_t inner_loops = 0;)
