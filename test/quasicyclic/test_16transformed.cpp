...
#include "m4ri/m4ri.h"
#include "challenges/qc16transformed.h"
...
TEST(BJMM, qc16transformed2) {
	...
	static constexpr ConfigBJMM config(n, k, w, p, l, l1,
									   ...
									   w-4,
									   NUMBER_THREADS, true);
	BJMMSpecialDOOM<config> bjmm(ee, ss, A);
	bjmm.run();
...
TEST(BJMM, qc16transformedNormal) {
	// Normal Code without DOOM and without SDOOM
	mzd_t *AT = mzd_from_str(n, n-k, h);
	mzd_t *A = mzd_transpose(nullptr, AT);
	mzd_t *ss = mzd_from_str(1, n-k, s);
	mzd_t *ss_tmp = mzd_init(ss->ncols, ss->nrows);
