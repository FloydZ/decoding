...
TEST(BJMMOuter, t1161) {
	...
	std::cout << ee_T << "\n";
#pragma omp parallel default(none) shared(cout, ee, ss, A) num_threads(NUMBER_OUTER_THREADS) if(NUMBER_OUTER_THREADS != 1)
	{
#pragma omp critical
		{ std::cout << "starting bjmm: " << omp_get_thread_num() << "\n"; }
		static constexpr ConfigBJMM config(n, k, w, p, l, l1,
		                                   148, 12,
		                                   l1, l - l1,
		                                   w,
		                                   NUMBER_THREADS);
		BJMM<config, DecodingList2> bjmm(ee, ss, A);
