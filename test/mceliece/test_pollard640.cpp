...
TEST(BJMM, pollard640p1) {
	...
	static constexpr ConfigPollard config(G_n, G_k, G_w, 1, 10, 1 << 9);
...
TEST(BJMM, pollard640p2) {
	...
	static constexpr ConfigPollard config(G_n, G_k, G_w, 2, 15, 1 << 14);
	Pollard<config> pollard(ee, ss, A);
	pollard.run();
	EXPECT_GE(w, hamming_weight(ee));
