...
TEST(MCEliece_d2_outer, t640) {
	...
	auto aswd = 0;
	for (int i = 0; i < 10; ++i) {
		aswd += emz_d2_outer<G_n, 70, G_k, 30, G_w, G_p, G_d, 15, G_w1, G_l2, G_w2, G_epsilon, 0, 15, 4, 1, 500>(ee, ss,
		                                                                                                         A);
...
TEST(MCEliece_d2, t640) {
	...
	for (int i = 0; i < 1; ++i) {
		std::cout << i << "\n";
		aswd += emz_d2<G_n, 0, G_k, G_l, G_w, G_p, G_d, G_l1, G_w1, G_l2, G_w2, G_epsilon, 1, 15, 1>(ee, ss, A);
		EXPECT_EQ(w, hamming_weight(ee));
	}
